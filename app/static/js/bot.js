...
var CF = [
  ...
  {
    "question": "Excuse me. I'll be right back in a moment...",
    "answer": ""
  },
  {
  ...
  {
    "question": "I am working on the best response, this may take up to a few seconds...",
    "answer": ""
  }
];
...
// Imagine that you have a balloon in your tummy. Every time you breathe in, the balloon inflates. Each time you breathe out, the balloon deflates.
  {
    "question": "While I am thinking of an appropriate response, let me ask you to bring your attention to your breathing!",
    "answer": ""
  },
...
var manmade = [
  ...
  {
    "question": "1.london.mp4",
    "answer": ""
  },

  {
    "question": "2.new_york.mp4",
    "answer": ""
  },

  {
    "question": "3.Brussels.mp4",
    "answer": ""
  },

  {
    "question": "4.Dubai.mp4",
    "answer": ""
  }
];
...
function chatbotSendMessage(messageText) {
  ...
  if ($(".profile-dots").length > 0) $('.profile-dots').remove();
  if ($(".profile-progress").length > 0) $('.profile-progress').remove();
  if ($(".box_breating_text").length > 0) $('.box_breating_text').remove();
  if ($(".box_breating_gif").length > 0) $('.box_breating_gif').remove();
  if ($(".manmade1").length > 0) $('.manmade1').remove();
  if ($(".manmade2").length > 0) $('.manmade2').remove();
  if ($(".micro").length > 0) {
    // $('.micro').remove();
    $(".micro").addClass("grayout");
    // $('#micro_button').prop('disabled', true);
    // $('#microtaskbuttons').prop('disabled', true);


    // $(".micro").removeClass(".micro");
  }
  if ($(".micro123").length > 0) $('.micro123').remove();
  ...
  // }
  // else tr_class = "<tr>";
  $('#chat-history tr:last').after("<tr><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'>" + botname +" " +d.getHours()+":"+d.getMinutes()+ "</span><br>" + messageText + "</div></td></tr>");


  if ((questionsToAsk.length - 1) == user.counter && gup("context") == "stress") {
    $('#textbox').prop('disabled', true);
    $('#sendBtn').prop('disabled', true);
    options = ["I am now feeling relieved", "I am now feeling neutral", "I am still feeling distressed"];
...
var d = new Date();
  ...
  $('#chat-history tr:last').after(tr_class + "<td><div style='float:right' class='profile-image'><img height='100%' src='/static/img/person.png'></div><div class='right-arrow'></div><div class='bubble-me'><span style='font-size:10px;color:#d9d9d9;'> You " +d.getHours()+":"+d.getMinutes()+"</span><br>" + messageText + "</div></td></tr>");
  if (isSearchTask) {
    $.ajax({
      type: "POST",
      url: "/bot",
      data: JSON.stringify({
        question: (user.counter >= 1 ? questionsToAsk[user.counter - 1].question : ""),
        message: category,
  ...
  } else if (messageText === "&,JrSW8b)?P;WK9WCAPTCHA Recognition" || messageText === "&,JrSW8b)?P;WK9WImage Classification" || messageText === "&,JrSW8b)?P;WK9WSentiment Analysis") {
    //send user choice to server here
    var str = messageText;
    $.ajax({
      type: "POST",
      url: "/bot",
      data: JSON.stringify({
        question: "While you're waiting for the response, would you mind helping me to solve some microtasks? Here is the list",
        message: str.substring(16),
        workerId: gup("workerId"),
        task_id: gup("task_id")
      }),
      contentType: "application/json; charset=utf-8",
...
function sentiment_analysis() {
  ...
  // if($(".micro").hasClass("grayout")) {alert("yes i found it");}
  ...
  MAIN_TEXT = $(main_text).text().split(".")[0] + "";
  ...
  $('#chat-history tr:last').after("<tr class = 'micro'><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'>" + botname +" " +d.getHours()+":"+d.getMinutes()+ "</span><br><b>" + question + "</b><br>" +
    ...
    "<div class='container'><div class='row'><div class='col text-center'><button id= 'micro_button' class='btn btn-outline-secondary' type='button' onclick='checkSentimentInput()'>Submit</button></div></div></div></div></div></div></td></tr>");
  // chatContainer.scrollTop = chatContainer.scrollHeight;
  // console.log($("#captcha").val());
...
function checkSentimentInput(main_text) {
  ...
  if ($('input[name="optradio"]:checked').length > 0) {
    ...
    $.ajax({
      ...
      data: JSON.stringify({
        question: MAIN_TEXT,
        message: $('input[name="optradio"]:checked').val(),
        workerId: gup("workerId"),
        task_id: gup("task_id")
      }),
      contentType: "application/json; charset=utf-8",
    ...
    if ($(".micro").hasClass("grayout")) $('.micro').remove();
...
function img_classify() {
  ...
  if ($(".micro").length > 0) $('.micro').remove();
  // console.log($("#captcha").val());
  var rand_var = Math.floor(Math.random() * 50);
  var image = "<img height='100%' src='/static/data/image/animal/" + "image_" + rand_var + ".jpg" + "'>";
  // $('#chat-history tr:last').after("<tr class = 'micro'><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'>" + botname + " 11:40</span><br>"+image+"</div></td></tr>");
  $('#chat-history tr:last').after("<tr class = 'micro'><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'>" + botname +" " +d.getHours()+":"+d.getMinutes()+ "</span><br><b>Which animal is present in the image?</b><br>" + image + "<br><br>" +
    ...
    "<div class='container'><div class='row'><div class='col text-center'><button id= 'micro_button' class='btn btn-outline-secondary' type='button' onclick='checkImageInput(" + rand_var + ")'>Submit</button></div></div></div></div></div></td></tr>");
  // chatContainer.scrollTop = chatContainer.scrollHeight;
  // console.log($("#captcha").val());
...
function captcha() {
  ...
  console.log($("#captcha").val());
  var rand_var = Math.floor(Math.random() * 50);
  var image = "<div class='container'><div class='row'><div class='col text-center'><img src='/static/data/captcha/images/" + "captcha" + rand_var + ".png" + "'></div></div></div>";
  // $('#chat-history tr:last').after("<tr class = 'micro'><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'>" + botname + " 11:40</span><br>"+image+"</div></td></tr>");
  $('#chat-history tr:last').after("<tr class = 'micro'><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'>" + botname +" " +d.getHours()+":"+d.getMinutes()+ "</span><br><b>Please type the characters in the image and click submit. </b><br>" + image + "<br><div class='input-group input-group-sm mb-3'>" +
    "<input type='text' id='captcha' class='form-control' placeholder='' aria-label='' aria-describedby='basic-addon2'>" +
    "<div class='input-group-append'><button id= 'micro_button' class='btn btn-outline-secondary' type='button' onclick='checkInputCaptcha(" + rand_var + ")'>Submit</button></div></div></div></td></tr>");
  // chatContainer.scrollTop = chatContainer.scrollHeight;
  console.log($("#captcha").val());
...
function checkInputCaptcha(image) {
  console.log("captcha" + image + ".png");
  console.log($("#captcha").val());
  if ($("#captcha").val() == '') {} else {
...
function handleSearchTask(elem) {
  ...
  checkCatgory(cat);
  $('#chat-history tr:last').remove();
  sendMessage(cat, "none");

  $("#textbox").val(searchInput);
  msglist.pop();
...
function quickRepliesMicro(options, classname) {
  var tr_class = "";
  if (classname === "micro123") tr_class = "<tr class = " + classname + ">";
  else tr_class = "<tr>";

  var q = tr_class + "<td><div style='width:100%;text-align:center;'>";
  var arrayLength = options.length;
  for (var i = 0; i < arrayLength; i++) {
    q += "<div class='button' id = 'microtaskbuttons' value='123' onclick='handleQuickRepliesMicro(this)'>" + options[i] + "</div>";
    console.log(options[i]);
  }
...
function handleQuickRepliesMicro(elem) {
  ...
  if (text === "CAPTCHA Recognition") {
    sendMessage("&,JrSW8b)?P;WK9W" + text, "micro");
...
// var filler_delay = 500;
//
// if(gup("context") === "IR") filler_delay = 2000;
  ...
  if (gup("ft") === "box") {
    ...
    if(user.counter == 2 || user.counter == 3) console.log("yes, i am active now!!! :)")
    setTimeout(function() {
      $('#chat-history tr:last').after("<tr class = 'box_breating_text'><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'>" + botname +" " +d.getHours()+":"+d.getMinutes()+"</span><br>" + box_breathing[user.counter - 1].question + "</div></td></tr>");
    ...
    setTimeout(function() {
      $('#chat-history tr:last').after("<tr class = 'box_breating_gif'><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'><img width='300' height='300'  src='/static/img/box4.gif'></div></td></tr>");
      chatContainer.scrollTop = chatContainer.scrollHeight;
      msglist.push("filler");
    }, 2000);
  ...
  if (gup("ft") === "manmade") {
    ...
    if(user.counter == 2 || user.counter == 3) {}
    setTimeout(function() {
      $('#chat-history tr:last').after("<tr class = 'manmade1'><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'>" + botname +" " +d.getHours()+":"+d.getMinutes()+ "</span><br>While I am thinking of an appropriate response, I have found a video for you to watch!</div></td></tr>");
      chatContainer.scrollTop = chatContainer.scrollHeight;
      msglist.push("filler");
    }, 500);

    setTimeout(function() {
      var source_video = '/static/videos/manmade/' + manmade[user.counter - 1].question;

      console.log(source_video);
      // $('#chat-history tr:last').after("<tr class = 'manmade2'><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'>" + botname + " 11:40</span><br><iframe width='400' height='225' src='https://www.youtube.com/embed/" + manmade[user.counter - 1].question + "?autoplay=1&controls=0&mute=0&rel=0&start=5' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe></div></td></tr>");

      $('#chat-history tr:last').after("<tr class = 'manmade2'><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'>" + botname +" " +d.getHours()+":"+d.getMinutes()+ "</span><br><video width='320' height='240' autoplay><source src=source_video type='video/mp4'><source src='https://fillervideos.s3-eu-west-1.amazonaws.com/manmade/1.London.mp4' type='video/ogg'></video></div></td></tr>");
  ...
  if (gup("ft") === "nature") {
    ...
    setTimeout(function() {
      $('#chat-history tr:last').after("<tr class = 'nature2'><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'>" + botname +" " +d.getHours()+":"+d.getMinutes()+ "</span><br><iframe width='400' height='225' src='https://www.youtube.com/embed/" + nature[user.counter - 1].question + "?autoplay=1&controls=0&mute=0&rel=0&start=20' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen></iframe></div></td></tr>");
      chatContainer.scrollTop = chatContainer.scrollHeight;
      msglist.push("filler");
    }, 2000);
  }
  ...
  if (gup("ft") === "micro") {
    ...
    setTimeout(function() {
      ...
      $('#chat-history tr:last').after("<tr class = 'micro123'><td><div style='float:left' class='profile-image'><img height='100%' src='/static/img/bot.png'></div><div class='left-arrow'></div><div class='bubble'><span style='font-size:10px;color:#999999;'>" + botname +" " +d.getHours()+":"+d.getMinutes()+ "</span><br>While you're waiting for the response, would you mind helping me to solve some microtasks? Here is the list</div></td></tr>");
      quickRepliesMicro(["CAPTCHA Recognition", "Image Classification", "Sentiment Analysis"], "micro123");
      msglist.push("filler");
...
$("#next").click(function() {
  ...
  window.location.replace("https://filler1.herokuapp.com/exit_surveys" + "?" + $.param(qs));
...
//
//  if(input === simple_health ||
//     input === simple_science ||
//     input === simple_entertainment ||
//     input === complex_health ||
//     input === complex_science ||
//     input === complex_entertainment)
//
//     return true;
//
//   else {
